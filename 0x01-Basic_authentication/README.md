...
* Create the class `Auth`:
	...
	* class name `Auth`
	* public method `def require_auth(self, path: str, excluded_paths: List[str]) -> bool:` that returns `False` - `path `and e`xcluded_paths` will be used later, now, you don’t need to take care of them
...
[5. Request validation!](./api/v1/app.py), [auth](./api/v1/auth/auth.py)
...
[6. Basic auth](./api/v1/app.py), [basic_auth](./api/v1/auth/basic_auth.py)
